<template>
  <div>
    <div class="cabecalho">
      <div><h1>Relatorios</h1><p>Vendas por unidade e onde cada produto esta guardado.</p></div>
    </div>
    ...
    <div class="painel">
      ...
      <div v-else class="tabela-rolagem">
        <table class="lista">
          <thead><tr><th>Unidade</th><th>Tipo</th><th>Vendas</th><th>Itens</th><th>Total</th></tr></thead>
          <tbody>
    ...
    <div class="painel">
      <div class="painel-topo">
        <h2>Onde esta o estoque</h2>
        <div class="linha-acoes">
          <input v-model="filtroEstoque" class="campo" style="max-width:240px" type="search" placeholder="Produto ou unidade" />
      ...
      <div v-else class="tabela-rolagem">
        <table class="lista">
          <thead><tr><th>Unidade</th><th>Tipo</th><th>Produto</th><th>Quantidade</th></tr></thead>
          <tbody>
...
function baixar(qual: string) {
  const linhas = qual === 'vendas'
    ? [['Unidade', 'Tipo', 'Vendas', 'Itens', 'Total'], ...vendas.value.map(v => [v.unit_name, v.unit_type, v.vendas, v.itens, v.total])]
    : [['Unidade', 'Tipo', 'Produto', 'Quantidade'], ...estoqueFiltrado.value.map(e => [e.unit_name, e.unit_type, e.product_title, e.qty])]
